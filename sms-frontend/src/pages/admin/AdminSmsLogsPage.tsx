...
import { useGetAdminSmsLogsQuery } from '../../api/endpoints/smsLogApi';
...
export function AdminSmsLogsPage() {
  ...
  const { data, isLoading, isError } = useGetAdminSmsLogsQuery(params);

  const columns: Column<SmsLog>[] = [
    { key: 'phone', header: 'Telefon', render: (l) => <span className="mono">{l.phone || '—'}</span> },
    { key: 'memberId', header: 'Aʼzo', render: (l) => <span className="mono muted">{l.memberId ? l.memberId.slice(-8) : '—'}</span> },
    {
      key: 'message',
      header: 'Xabar',
      render: (l) => (
        <span className="muted" style={{ display: 'inline-block', maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {l.message || '—'}
        </span>
      ),
    },
  ...
  return (
    <>
      <PageHeader title="Global SMS jurnali" subtitle="Barcha aʼzolarning SMS tarixi." />
      <Card bodyless>
